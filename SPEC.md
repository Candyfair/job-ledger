...
4. **Redirects immediately to `/dashboard?runId=<id>`** — both authenticated and anonymous (changed 2026-09-11, supersedes the 2026-09-07 "bare `/dashboard` for authenticated" decision: landing on the "all time" aggregate gave no way to tell which listings the just-triggered run actually produced). Progress is then shown via the dashboard's status banner — see below.
...
4. Run-history strip (authenticated only) is clickable per entry to filter the listings below to that run; no entry selected shows the "all time" aggregate. (Currently broken — see §9.) Initial page: the 6 most recent runs (decided 2026-09-11); "Charger plus" (existing cursor pagination) reveals the rest.
...
Claim mechanism: if sign-up/sign-in completes while a `runId` was carried through either entry point, the corresponding `ScrapeRun` — that single run only — is reattached to the account: `userId` is set from `NULL` to the new user's id. This is idempotent — a run already attached to an account (its own or anyone else's) is never reassigned; a repeat or late claim attempt is simply a no-op. No `JobConfig` is created from the claimed run's ad hoc parameters — the saved search still has to be configured manually on `/` afterward, no longer the landing page for this flow (see below). No verification confirms the claimant was the original visitor; this is an accepted risk given the low sensitivity of job-listing data.

**Landing target (changed 2026-09-11, supersedes "lands on `/`"):** the post-sign-in redirect for a run-carrying claim is `/dashboard?claimRunId=<id>`, not `/` — landing on the empty trigger form gave no visible sign the run had carried over. `RunClaimOnMount` (mounted on both `/` and `/dashboard`) awaits the claim POST before navigating, then replaces the URL with `?runId=<id>` on success so the dashboard shows the just-claimed run explicitly rather than the "all time" aggregate. `app/dashboard/page.tsx`'s authenticated branch does not bounce a zero-runs visitor to `/` while `claimRunId` is present, since the claim (and therefore that first owned run) only completes after this component mounts client-side. A run-less sign-in with no `runId` in context is unaffected — it still lands on `/`.
...
6. **Run status rollup** (built 2026-09-10 — supersedes both the `await`-based proposal in §9 and the earlier "each task recomputes from combined `Listing`/`SiteStatus` state" sketch). The run's full expected membership is the `ScrapeRunSite` table (DATA_MODEL.md) — one row per `(scrapeRunId, site, jobConfigId)` task, pre-created with `outcome='pending'` by `POST /api/scrape/trigger` in the same transaction as the `ScrapeRun` row, before any task is enqueued. Each site task, on finishing (success, empty extraction, failure, or kill-switch skip), writes **only its own** `ScrapeRunSite` row — lock-free, the key is per-task — then calls the shared `recomputeAndWriteRunStatus` (`lib/run-status/`). That function is the **sole** writer of `ScrapeRun.status`: it takes a `SELECT … FOR UPDATE` lock on the `ScrapeRun` row, combines outcomes in two tiers — `combineSiteOutcome` across a site's `JobConfig`s (precedence `pending > failed > empty_extraction > completed > skipped`), then `combineRunStatus` across sites (any `pending` → `running`; else any `failed` or `empty_extraction` → `partial_failure`; else any `completed` → `completed`; else all-`skipped` → `partial_failure`) — and writes the result only if it changed. `POST /api/scrape/trigger` (§7) never awaits a task; the pre-created `pending` rows are what make "at least one pending ⇒ running" reliable without counting in-flight tasks. A task that dies without writing its row leaves it `pending`; `trigger/finalize-stale-runs.ts` sweeps runs still `running` 15 min on, flips their leftover `pending` rows to `outcome='failed'` / `failureCause='timeout'`, and calls the same recompute.

**Apec exception (decided 2026-09-08):** Apec.fr's results page is an Angular SPA backed by its own search web-service, which returns fully structured per-listing JSON plus a `totalCount`. Apec therefore skips steps 1–4 above — no Playwright, no LLM listing extraction — and calls that service directly (`lib/scraping/apec-api.ts`, `runApecApiScrape`). It still uses the LLM for exactly one thing: a single batched `roleCanonical` call (`RoleCanonicalizer.canonicalizeRoles`), so the cross-site dedup signal (§5) is produced the same way for Apec and HelloWork. `companyNormalized` for Apec is a deterministic transform (`lib/dedup/normalize-company.ts`). This does not weaken CLAUDE.md decision #1 — no LLM _judgment_ is replaced by code; Apec's fields simply arrive already structured. HelloWork is unchanged. A Zod schema on the response is the drift tripwire that plays the role selector timeouts play on the Playwright path. Pagination is bounded by `totalCount`, making the previous "results page never terminates" failure structurally impossible.
...
- `POST /api/scrape/trigger` — creates the `ScrapeRun` row plus one `pending` `ScrapeRunSite` row per fan-out unit (§4) in one transaction, invokes the per-site Trigger.dev task(s), returns `runId`. Authoritative `ScrapeRun` creation lives here: the endpoint passes the run id into each site task, which appends `Listing` rows, updates its own `ScrapeRunSite` row, and then calls the shared `recomputeAndWriteRunStatus` (§4) — it never writes `ScrapeRun.status` directly. A site task invoked **without** a run id — Trigger.dev's Test tab, the `scripts/test-scrape-*` harnesses — falls back to creating its own single-site `ScrapeRun` (and, on the success/skip paths, its one `ScrapeRunSite` row); that path is a testing convenience, not the production flow. A standalone task that _fails_ records nothing — `SiteStatus` only, no orphan `ScrapeRun`.
...
- `GET /api/scrape/status/:runId` — polled by the frontend for in-app status, now active for both anonymous and authenticated dashboard views (§3). Ownership: an anonymous caller only sees `userId IS NULL` runs; an authenticated caller additionally sees their own runs — never a foreign authenticated user's run, and the two failure modes (nonexistent vs. not-yours) are indistinguishable (404 either way) so existence is never leaked.
  ...
  ```
  {
    ...
    sites: {
      ...
      status: "pending" | "completed" | "empty_extraction" | "failed" | "skipped";
      failureCause: "markup_broken" | "bot_challenge" | "timeout" | null;
      listingCount: number;
    }[];
    kept: number;
    excluded: number;
  ...
  **Status is a persisted fact** (see §4 for the write mechanism). `status` is read straight from `ScrapeRun.status`, kept current by the run-status rollup — not derived on read. `sites[].status` is the tier-1 `combineSiteOutcome` for that site; `empty_extraction` and `skipped` are new values whose user-facing copy (banner, per-site line) is deferred to the navigation/banner work — only the data shape changes here. `lib/dashboard/derive-run-status.ts` (the old live heuristic over `Listing` + global `SiteStatus`) is deleted; `lib/dashboard/assemble-run-status.ts` builds this payload from the persisted `ScrapeRun.status` + `ScrapeRunSite` rows (per-site `status` / `listingCount`) + `Listing` rows (`kept` / `excluded` / `duplicateGroups`).

- `GET /api/scrape/runs` — authenticated-only, cursor-paginated "load more" for the dashboard's run-history strip (Session 6). 401 without a session. The strip's _initial_ page (6 most recent runs, decided 2026-09-11 — see §6) is capped by the SSR caller (`app/dashboard/page.tsx`) passing an explicit `limit`, not by this endpoint — every subsequent "load more" page from here stays at the larger default page size.
...
- `POST /api/scrape/claim` — the claim endpoint (decided 2026-09-07, implemented as a dedicated route — `app/api/scrape/claim/route.ts`): invoked client-side (`RunClaimOnMount`) as part of the sign-up/sign-in flow when a `runId` was carried through as `?claimRunId=` via `callbackURL` (§3). Requires a session (401 otherwise). Given a `runId` and the caller's session, sets `ScrapeRun.userId` from `NULL` to the session's user id; otherwise (already claimed) a no-op — always 200 either way.
- Trigger.dev task → writes directly to Postgres on completion (no callback to Next.js needed); updates its own `ScrapeRunSite` row and calls the run-status recompute (§4).
...
- **Run status rollup**: `combineSiteOutcome` / `combineRunStatus` precedence is correct across every outcome combination and order-independent; the all-`skipped` run resolves to `partial_failure`. `recomputeAndWriteRunStatus` groups `ScrapeRunSite` rows by site, applies both tiers, writes only on a change, and takes a `FOR UPDATE` lock; 3+ tasks across 2 sites with mixed `JobConfig`s completing in any order produce the same final `ScrapeRun.status`. `POST /api/scrape/trigger` pre-creates one `pending` `ScrapeRunSite` row per resolved site×`JobConfig` pair, in the `ScrapeRun` transaction, before any task is enqueued, and returns before any task completes (asserted via mock timing). A kill-switch skip on the shared fan-out path updates only its own `ScrapeRunSite` row, never `ScrapeRun.status` directly. An empty extraction (0 listings, no Playwright error) records `empty_extraction`, not `completed`. `GET /api/scrape/status/:runId` reads the persisted status, not a heuristic.
...
- **Run status rollup — implemented 2026-09-10** (mechanism: §4, §7). `ScrapeRun.status` is a persisted fact written only by `recomputeAndWriteRunStatus` (`lib/run-status/`) from per-task `ScrapeRunSite` rows. The three former blind spots are resolved: (1) a kill-switch skip on the shared fan-out path is a real `outcome='skipped'` row — `recordKillSwitchSkip` no longer leaves the shared run untouched (the Session 6 gap); (2) an empty or degraded extraction with no Playwright error is a real `outcome='empty_extraction'` row → `partial_failure`, on every path including the multi-site fan-out; (3) `SiteStatus` is no longer consulted for run status at all, so its global-singleton nature can't misattribute a failure between two runs targeting the same site — each run has its own `ScrapeRunSite` rows. `trigger/finalize-stale-runs.ts` is retained but reworked: it no longer writes `ScrapeRun.status` directly — it flips a stuck run's leftover `pending` rows to `outcome='failed'` / `failureCause='timeout'` and calls the same recompute, so a genuinely-finished run it happens to catch is still resolved correctly (not blindly forced to `partial_failure`).
